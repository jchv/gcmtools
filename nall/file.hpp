...
namespace nall {
  ...
  // simpler nall::file by nmn
  // prefers chunked read/write over while loops
  // does not do buffering internally
  struct file {
    ...
    void read(uint8_t *buffer, unsigned length) {
      file_offset += fread(buffer, 1, length, fp);
    }
    ...
    void write(const uint8_t *buffer, unsigned length) {
      file_offset += fwrite(buffer, 1, length, fp);
    }

    template<typename... Args> void print(Args... args) {
      string data(args...);
      const char *p = data;
      write((const uint8_t *)p, strlen(p));
    }
    ...
    void seek(int offset, index index_ = index::absolute) {
      ...
      if(fseek(fp, req_offset, SEEK_SET) == 0)
...
/*
  struct file {
    ...
    bool open(const string &filename, mode mode_) {
      nall::print("Opening file ", filename, "\n");
      if(fp) return false;
      nall::print("Not already open.\n");
